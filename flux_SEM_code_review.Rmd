...
#create a hypsometric table with depth and volume (Hounshell et al. 2020, SI2 and SI3) I need some more explanation on how the depths were chosen in the Hounshell et al. 2020 table and how they calculated the volumes for each layer. If you use Hounshell et al table then I think the sample right below the surface should just use the total volume and not split it as you have. Realistically what depths should make up a layer?

# Have you looked at the FCR baythmetry data package which has volumes for many different layers. 
# Looking at these volumes in the table
...
#Note: this is just for temp_5m and iCH4_5m but did the same for all variables of interest.

ccf(data_flux$temp_5m, data_flux$iCH4_5m, lag.max = 5)
...
Diffusion/Ebullition with physiological drivers model: In this concept, physiological drivers were added to the diffusion/ebullition model.


```{r}
#Diffusion/Ebullition with Physiological drivers
#fit the path models
model_flux <- psem(
...
Diffusion/Ebullition with physiological and biological drivers model: In this concept, physiological and biological drivers were added to the diffusion/ebullition model.


```{r}
#Diffusion/Ebullition with Physiological and Biological drivers
#fit the path models
model_flux <- psem(
  lm(daily_CH4 ~ Wind + iCH4_5m + temp_1.6m_lag3 + iCH4_1.6m + iCH4_9m + sch_stab + mean_Chla, data_flux),
  lm(iCH4_5m ~ DOsat_5m + temp_5m + mean_fDOM_lag1 + iCH4_9m + sch_stab, data_flux)
...
model_flux <- '
  ...
  iCH4_5m ~ DOsat_5m + temp_5m + mean_fDOM_lag1 + iCH4_9m + sch_stab
